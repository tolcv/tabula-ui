...
var pathParts = window.location.pathname.replace(base_uri || '/', '').split('/');
var hashParts = (window.location.hash || '').replace(/^#\/?/, '').split('/');
PDF_ID = pathParts[0] === 'pdf' ? pathParts[1] : (hashParts[0] === 'pdf' ? hashParts[1] : null);
...
Tabula.Page = Backbone.Model.extend({
  ...
  imageUrl: function(){
    var resolution = 800;
    this.set('image_url', (base_uri || '/') + 'api/documents/' + PDF_ID + '/thumbnail/' + this.get('number'));
...
Tabula.Pages = Backbone.Collection.extend({
  ...
  comparator: 'number',
  initialize: function(){
    this.url = (base_uri || '/') + 'api/documents/' + PDF_ID + '/pages';
  }
});
...
Tabula.Document = Backbone.Model.extend({
  ...
  initialize: function(options){
    ...
    this.url = (base_uri || '/') + 'api/documents/' + this.pdf_id;
...
Tabula.AutodetectedSelections = Tabula.Selections.extend({
  ...
  initialize: function(){
    this.url = (base_uri || '/') + 'api/documents/' + PDF_ID + '/detect-tables';
    this.autodetectPending = true;
    _.bindAll(this, 'updateOrCreateByVendorSelectorId');
  },

  parse: function(response){
    // Backend returns {status:'pending', tables: []} while the job is still running.
...
Tabula.Query = Backbone.Model.extend({
  ...
  doQuery: function(options) {
    ...
    console.log('[extract] request', {
      documentId: PDF_ID,
      endpoint: (base_uri || '/') + 'api/documents/' + PDF_ID + '/extract',
      coordCount: this.get('list_of_coords') ? this.get('list_of_coords').length : 0,
      extractionMethod: this.get('extraction_method')
    });
    $.ajax({
        type: 'POST',
        url: (base_uri || '/') + 'api/documents/' + PDF_ID + '/extract',
        data: this.query_data,
        success: _.bind(function(resp) {
          console.log('[extract] success', {
            documentId: PDF_ID,
            responseItems: resp && resp.length ? resp.length : 0
          });
          this.set('data', resp);
  ...
  _.extend({
    ...
    checkForAutodetectedTables: function(){
      this.pdf_document.autodetected_selections.fetch({
        success: _.bind(function(){
          var isPending = this.pdf_document.autodetected_selections.autodetectPending;
          if (isPending) {
            this.hasAutodetectedTables = false;
        ...
        error: _.bind(function(){
          console.log("autodetected tables request failed; retrying");
          this.autodetect_timer = window.setTimeout(this.checkForAutodetectedTables, 2000);
          this.render();
        }, this)
      });
    ...
    removePage: function(removedPageModel){
      $.ajax({
        type: 'DELETE',
        url: (base_uri || '/') + 'api/documents/' + PDF_ID + '/page/' + removedPageModel.get('number'),
        success: function() { Tabula.pdf_view.pageCount -= 1; }
      });
