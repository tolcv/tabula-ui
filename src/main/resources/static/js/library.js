...
Tabula.FileUpload = Backbone.Model.extend({
  ...
  checkStatus: function() {
    if(typeof this.get('file_id') == 'undefined'){
      this.pct_complete = 1;
      this.message = "waiting to be processed..."
    }else{
      $.ajax({
          dataType: 'json',
          url: (base_uri || '/') + 'api/queue/document/' + this.get('file_id'),
          success: _.bind(function(jobs, status, xhr) {
            // jobs is an array of Job objects: [{id, documentId, type, status, progress, errorMessage}]
            var allDone = jobs.length > 0 && _(jobs).all(function(j){ return j.status === 'COMPLETED'; });
            var anyFailed = _(jobs).any(function(j){ return j.status === 'FAILED'; });
            var avgProgress = jobs.length > 0 ? Math.round(_(jobs).reduce(function(sum, j){ return sum + (j.progress || 0); }, 0) / jobs.length) : 0;

            var data = {};
            if (anyFailed) {
              data.status = 'error'; data.error_type = 'unknown'; data.pct_complete = 99; data.message = ''; data.warnings = [];
            } else if (allDone) {
              data.status = 'completed'; data.pct_complete = 100; data.message = 'complete'; data.warnings = [];
            } else {
              data.status = 'working'; data.pct_complete = avgProgress; data.message = 'processing'; data.warnings = [];
            }
...
Tabula.UploadedFilesCollection = Backbone.Collection.extend({
    ...
    url: function(){ return 'api/documents'+ '?' + Number(new Date()).toString() },
...
Tabula.UploadedFileView = Backbone.View.extend({
  ...
  deletePDF: function(e) {
    ...
    $.ajax({
      type: 'DELETE',
      url: (base_uri || '/') + 'api/documents/' + pdf_id,
      success: function() {
    ...
    uploadPDF: function(e){
      ...
      $.ajax({
          ...
          success: _.bind(function (res) {
              var statuses = _.isString(res) ? JSON.parse(res) : res;
              if (!_.isArray(statuses)) {
                console.log('[upload] unexpected response payload', res);
                return;
              }
